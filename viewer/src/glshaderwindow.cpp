...
// Initialize obvious default values here (e.g. 0 for pointers)
    : OpenGLWindow(parent), modelMesh(0),
      m_program(0), ground_program(0), skel_program(0), compute_program(0), shadowMapGenerationProgram(0),
      g_vertices(0), g_normals(0), g_texcoords(0), g_colors(0), g_indices(0),
      sk_vertices(0), sk_normals(0), sk_texcoords(0), sk_colors(0), sk_indices(0),
      ...
      m_indexBuffer(QOpenGLBuffer::IndexBuffer), ground_indexBuffer(QOpenGLBuffer::IndexBuffer), skel_indexBuffer(QOpenGLBuffer::IndexBuffer), isproject(false)
{
...
glShaderWindow::~glShaderWindow()
{
    ...
    skel_vertexBuffer.release();
    skel_vertexBuffer.destroy();
    skel_indexBuffer.release();
    skel_indexBuffer.destroy();
    skel_colorBuffer.release();
    skel_colorBuffer.destroy();
    //skel_normalBuffer.release();
    //skel_normalBuffer.destroy();
    //skel_texcoordBuffer.release();
    //skel_texcoordBuffer.destroy();
    skel_vao.release();
    skel_vao.destroy();
    if (sk_vertices) delete [] sk_vertices;
    if (sk_colors) delete [] sk_colors;
    //if (sk_normals) delete [] sk_normals;
    if (sk_indices) delete [] sk_indices;
...
// keyPressEvent -> if space toggleAnimating()
// in toggleAnimating() : m_animating = !m_animating; m_update_pending = false;
// -> call renderNow()
// in renderNow() :
// if m_context not exists, create it and initialize
// then call to render() (defined in glshaderwindow.cpp)
// // render() is a call to
// if m_animating, call to renderLater()
// in renderLater():
// if not m_update_pending, { m_update_pending = true;
...
void glShaderWindow::keyPressEvent(QKeyEvent* e)
{
	...
	switch (key)
	{
	case Qt::Key_Space: // if we press space then we move forward in the keyframe succession
		toggleAnimating();
		break;
	default:
		break;
...
void glShaderWindow::bindSceneToProgram()
{
    ...
    std::cerr << "in bindopenscene model" << std::endl; 
    ...
    // **************** skel
    // *********** REDEFINE skeleton buffers
	rootNode->joint_vertices.clear();
	std::cerr << "initposition" << std::endl;
	rootNode->initPosition();
	skel_vao.bind();
	skel_vertexBuffer.setUsagePattern(QOpenGLBuffer::StaticDraw);
	skel_vertexBuffer.bind();

	sk_numPoints = 2*(Joint::total_joint_number -1); // number of joints 16
	sk_numIndices = 2*(Joint::total_joint_number -1);
	sk_numPoints = 2*(Joint::total_joint_number -1); // number of joints 16
	// Allocate once, fill in for every new model.
	if (sk_vertices == 0) sk_vertices = new trimesh::point[sk_numPoints];
	//if (sk_normals == 0) sk_normals = new trimesh::vec[sk_numPoints];
	if (sk_colors == 0) sk_colors = new trimesh::point[sk_numPoints];
	//if (sk_texcoords == 0) sk_texcoords = new trimesh::vec2[sk_numPoints];
	if (sk_indices == 0) sk_indices = new int[sk_numPoints]; // for each joint thats not the root : 2 point to get a line

	for (int i = 0; i<sk_numPoints; i++) {
		sk_vertices[i] = Joint::joint_vertices[i]; // they're already grouped two by two
		//std::cerr << sk_vertices[i] << std::endl;
		//std::cerr << (sk_vertices[i])[0] << " " << (sk_vertices[i])[1] << " " << (sk_vertices[i])[2] << std::endl;
		//std::cerr << std::endl;
		sk_indices[i] = i; //trivial with GL_LINES
		sk_colors[i] = trimesh::point(0.8, 0.5, 0.5, 1.0);
	}


	skel_vertexBuffer.setUsagePattern(QOpenGLBuffer::StaticDraw);
	skel_vertexBuffer.bind();
	skel_vertexBuffer.allocate(sk_vertices, sk_numPoints * sizeof(trimesh::point));

	skel_colorBuffer.setUsagePattern(QOpenGLBuffer::StaticDraw);
	skel_colorBuffer.bind();
	skel_colorBuffer.allocate(sk_colors, sk_numPoints * sizeof(trimesh::point));


	skel_indexBuffer.setUsagePattern(QOpenGLBuffer::StaticDraw);
	skel_indexBuffer.bind();
	skel_indexBuffer.allocate(sk_indices, sk_numIndices * sizeof(int));


	skel_program->bind();
	skel_vertexBuffer.bind();
	skel_program->setAttributeBuffer( "vertex", GL_FLOAT, 0, 4 ); // 4 because 4D
	skel_program->enableAttributeArray( "vertex" );
	skel_colorBuffer.bind();
	skel_program->setAttributeBuffer( "color", GL_FLOAT, 0, 4 );
	skel_program->enableAttributeArray( "color" );

	skel_program->setUniformValue("noColor", false);
	skel_program->release();

	// no shadow map
	skel_vao.release();
	// ********* END REDEFINE SKEL buffers
        // **************** end skel
...
void glShaderWindow::initialize()
{
    ...
    timerId = startTimer(60); // timer event every x millisecond
    std::cerr << "frame number " << timerId << std::endl;

    std::cerr << "before" << std::endl;
    rootNode = Joint::createFromFile("walkSit.bvh");
    ...
    skel_program = prepareShaderProgram(shaderPath + "3_textured.vert", shaderPath + "3_textured.frag"); // SKELETON
    if (shadowMapGenerationProgram) {
        shadowMapGenerationProgram->release();
        delete(shadowMapGenerationProgram);
    }
    ...
    skel_vao.create();
    skel_vao.bind();
    skel_vertexBuffer.create();
    skel_indexBuffer.create();
    skel_colorBuffer.create();
    //skel_normalBuffer.create();
    //skel_texcoordBuffer.create();
    skel_vao.release();
...
{
	//std::cerr << "curr value of timerId is " << timerId << std::endl;
	// IF MAXFRAME EXCEEDED THEN REGO TO FRAME 0 CLEAR STUFF

	// *********** REDEFINE skeleton buffers
	if (manimate) {
		if (mframe_id >= this->rootNode->_dofs[0]._values.size()) { // if we arrive at the end reset frame_id to 0 to loop
			mframe_id = 0;
		}

		manimate = false; // avoid data races
		std::cerr << "frame number : " << mframe_id << std::endl;
		//this->rootNode->joint_vertices.clear()
		this->rootNode->animate(mframe_id);
		//std::cerr << "number of frame for rootNode " << (this->rootNode->_dofs[4])._values.size() << std::endl;
		skel_vao.bind();
		skel_vertexBuffer.setUsagePattern(QOpenGLBuffer::StaticDraw);
		skel_vertexBuffer.bind();

		//int number_of_channels = this->rootNode->_dof;
		sk_numIndices = 2*(Joint::total_joint_number -1); // 24
		sk_numPoints = 2*(Joint::total_joint_number -1); // number of joints 16
		// Allocate once, fill in for every new model.
		if (sk_vertices == 0) sk_vertices = new trimesh::point[sk_numPoints];
		//if (sk_normals == 0) sk_normals = new trimesh::vec[sk_numPoints];
		...
		for (int i = 0; i<sk_numPoints; i++) {
			...
			sk_vertices[i] = Joint::joint_vertices[i]; // they're already grouped two by two
			sk_indices[i] = i; //trivial with GL_LINES
			sk_colors[i] = trimesh::point(0.2, 0.5, 0.2, 1);
		}
		//std::cerr << std::endl;
		...
		//skel_normalBuffer.bind();
		//skel_program->setAttributeBuffer( "normal", GL_FLOAT, 0, 4 );
		//skel_program->enableAttributeArray( "normal" );

		skel_program->setUniformValue("noColor", false);

		//skel_texcoordBuffer.bind();
		//skel_program->setAttributeBuffer( "texcoords", GL_FLOAT, 0, 2 );
		//skel_program->enableAttributeArray( "texcoords" );

		skel_program->release();
		skel_vao.release();
		// ********* END REDEFINE SKEL buffers

		//int max_frame_id = 5000; // get it from file, this is the number of frames in total
		timerId = (timerId + 1);
		mframe_id += 1;
		//std::cerr << mframe_id << std::endl;
		renderNow();
		manimate = true;
	}
}
...
void glShaderWindow::render()
{
    ...
    if (hasComputeShaders) {
		...
		// this is where we draw the mesh
		m_vao.bind();
		glDrawElements(GL_TRIANGLES, 3 * m_numFaces, GL_UNSIGNED_INT, 0);
		m_vao.release();
		// this is where we draw the ground
		ground_vao.bind();
    ...
    // this renders the model, useful for us? TODO
    m_vao.bind();
    //glDrawElements(GL_TRIANGLES, 3 * m_numFaces, GL_UNSIGNED_INT, 0);
    m_vao.release();
    m_program->release();
	...
	// draw the skeleton
	// also draw the skeleton, with a different shader program
        skel_program->bind();
        skel_program->setUniformValue("lightPosition", lightPosition);
        skel_program->setUniformValue("matrix", m_matrix[0]);
        skel_program->setUniformValue("lightMatrix", m_matrix[1]);
        skel_program->setUniformValue("perspective", m_perspective);
        skel_program->setUniformValue("normalMatrix", m_matrix[0].normalMatrix());
        skel_program->setUniformValue("lightIntensity", 1.0f);
        skel_program->setUniformValue("blinnPhong", blinnPhong);
        skel_program->setUniformValue("transparent", transparent);
        skel_program->setUniformValue("lightIntensity", lightIntensity);
        skel_program->setUniformValue("shininess", shininess);
        skel_program->setUniformValue("eta", eta);
        skel_program->setUniformValue("radius", modelMesh->bsphere.r);

	//if (skel_program->uniformLocation("colorTexture") != -1) skel_program->setUniformValue("colorTexture", 0);

        /*if (skel_program->uniformLocation("shadowMap") != -1) {
            skel_program->setUniformValue("shadowMap", 2);
            // TODO_shadowMapping: send the right transform here
        }*/

        skel_vao.bind();
	//glDrawElements(GL_LINES, sk_numIndices, GL_UNSIGNED_INT, 0); // TODO
        glDrawElements(GL_LINES, sk_numIndices, GL_UNSIGNED_INT, 0);
        skel_vao.release();
        skel_program->release();
	...
	/*if (ground_program->uniformLocation("colorTexture") != -1) ground_program->setUniformValue("colorTexture", 0);
        ...
        ground_vao.bind();
	// using glDrawElements(GL_LINES, g_numIndices, GL_UNSIGNED_INT, 0) TODO
        glDrawElements(GL_TRIANGLES, g_numIndices, GL_UNSIGNED_INT, 0); //not draw ground
        ground_vao.release();
        ground_program->release();
    }
}
